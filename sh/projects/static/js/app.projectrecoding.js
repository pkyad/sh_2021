...
app.controller('main', function($scope, $http, $interval, $uibModal, Flash, $sce,$timeout, $filter) {
    ...
    if ($scope.selector.idx != -1  && typeof event.data == 'object' && event.data.typ && event.data.typ == 'autofill') {
      ...
      if (f.type == 'date' ) {
        ...
        for (var j = 0; j < names.length; j++) {
          ...
          // if (typeof $scope.fieldValuesData[indx+j].pk == "number") {
          //   continue
          // }

          var res = $scope.addMore($scope.fieldValuesData[indx+j] , indx+j)
        ...
        $scope.fieldValuesData[$scope.selector.idx].txt = $scope.fieldValuesData[$scope.selector.idx].txt.substring(0 , $scope.fieldValuesData[$scope.selector.idx].maxLength)
      }
      $scope.$apply()
    }
  });
  ...
  $scope.nextFile = function(){
    ...
    then(function(response) {
      ...
      if (response.data.nxt) {
        ...
        window.location.href='/project/coder/'+projectpk
      }
    })
  }
  ...
  $scope.getFileData = function(){
    ...
    then(function(response) {
      ...
      if (response.data.length>0) {
        ...
        $scope.files()
        if (!$scope.parentField.relocked) {
          $http({
            method: 'PATCH',
            url: '/api/projects/fileupload/' + $scope.parentField.pk +'/',
            data:{relocked:true,updaterecodingVal:true}
          }).
          then(function(response) {
            console.log('re coding user value updateddddd');
          })
        }
      }else {
        console.log('empty');
  ...
  $scope.selectAll = function() {
    for (var i = 0; i < $scope.fieldValuesData.length; i++) {
      $scope.fieldValuesData[i].needCorrection = $scope.allSelected;
    }
  }
  ...
  $scope.files = function() {
    ...
    $http.get('/api/projects/fieldValue/?project=' + projectpk + '&file=' + $scope.parentField.pk+'&needCorrection=true').
    then(function(response) {
  ...
  $scope.resubmit = function() {
    ...
    for (var i = 0; i < $scope.fieldValuesData.length; i++) {
      ...
      if (!toSend.needCorrection) {
        console.log("Skipping : " , toSend);
        continue
      }
