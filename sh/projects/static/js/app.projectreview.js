...
app.controller('main', function($scope, $http, $interval, $uibModal, Flash, $sce,$timeout) {
  ...
  $scope.updateViewerUrl = function(url) {
    console.log("updating iframe with image url : " , url);
  ...
  $scope.nextFile = function(){
    ...
    then(function(response) {
      ...
      if (response.data.nxt) {
        window.location.href='/project/reqa/'+projectpk
      }else {
        $scope.getFileData()
      }
    })
  ...
  $scope.files = function() {
    ...
    then(function(response) {
      ...
      console.log(qaCanEdit,'can editttttttttttt');
      for (var i = 0; i < $scope.fieldValuesData.length; i++) {
        if (qaCanEdit) {
          $scope.fieldValuesData[i].editing = true
        }
        // $scope.fieldValuesData[i].dtMode='fullDate'
      }
  ...
  $scope.addMore = function(obj,idx){
    ...
    console.log($scope.multiCountMap);
  ...
  $scope.approve = function() {
    ...
    $http({
      ...
      data: {
        'status': 'approved',
        'checked' : true,
        'qcTime':$scope.timeInSec,
        'fieldsValArr' : fieldsValArr,
        'qcComment': comment,
      },
    }).
    then(function(response) {
      Flash.create('success', 'Approved Successfully');
