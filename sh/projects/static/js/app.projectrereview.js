...
app.controller('main', function($scope, $http, $interval, $uibModal, Flash, $sce,$timeout , $filter) {

  $scope.saving = false;
  $scope.qaCanEdit = qaCanEdit;
  ...
  $scope.nextFile = function(){
    $http({
      method: 'GET',
      url: '/api/projects/checkNextFile/?typ=reqc&projectPk='+projectpk
    }).
    then(function(response) {
      ...
      if (response.data.nxt) {
        ...
        window.location.href='/project/qa/'+projectpk
      }
    })
  }
  ...
  $scope.getFileData = function(){
    ...
    $scope.selector = {allSelected : false}

    $scope.comment = {'text': '','showBtn':false}
    $http({
      method: 'GET',
    ...
    then(function(response) {
      ...
      if (response.data.length>0) {
        ...
        if ($scope.fileUploadData[0].children.length>0) {
          ...
          console.log($scope.mergedData,'mergedDataaaaaaaaaaaaaaaaaa')
        }else {
          $scope.showNxtBtn = false
          $scope.showPrevBtn = false
        }
        ...
        $scope.files()
        if (!$scope.parentField.rechecklocked) {
          $http({
            method: 'PATCH',
            url: '/api/projects/fileupload/' + $scope.parentField.pk +'/',
            data:{rechecklocked:true,updatereqaVal:true}
          }).
          then(function(response) {
            console.log('qa user value updateddddd');
          })
        }
      }else {
  ...
  $scope.selectAll = function() {
    if ($scope.selector.allSelected) {
      $scope.comment.showBtn = true
    }else {
      $scope.comment.showBtn = false
    }
    for (var i = 0; i < $scope.fieldValuesData.length; i++) {
      $scope.fieldValuesData[i].needCorrection = $scope.selector.allSelected;
    }
  }

  $scope.checkSelection = function() {
    for (var i = 0; i < $scope.fieldValuesData.length; i++) {
      if ($scope.fieldValuesData[i].needCorrection) {
        $scope.comment.showBtn = true
        return
      }else {
        $scope.comment.showBtn = false
  ...
  $scope.files = function() {
    ...
    $http.get('/api/projects/fieldValue/?needCorrection=true&project=' + projectpk + '&file=' + $scope.parentField.pk).
    then(function(response) {
      $scope.fieldValuesData = response.data
      for (var i = 0; i < $scope.fieldValuesData.length; i++) {
        $scope.fieldValuesData[i].needCorrection = false
      }
      if (qaCanEdit) {
        for (var i = 0; i < $scope.fieldValuesData.length; i++) {
  ...
  $scope.finalSubmit = function() {
    ...
    for (var i = 0; i < $scope.fieldValuesData.length; i++) {
      ...
      // if ($scope.fieldValuesData[i].field.type == 'date' && $scope.fieldValuesData[i].dt.toJSON != undefined) {
      ...
      if ($scope.fieldValuesData[i].field.type == 'date' && $scope.fieldValuesData[i].dt != null) {
        try {
          if ($scope.fieldValuesData[i].dt.getHours()==0) {
            toSend.dt =  $filter('date')($scope.fieldValuesData[i].dt, "yyyy-MM-dd");
          }else {
            toSend.dt = $filter('date')($scope.fieldValuesData[i].dt, "yyyy-MM-dd");
          }
        } catch (e) {
  ...
  $scope.sendBack = function() {
    ...
    for (var i = 0; i < $scope.fieldValuesData.length; i++) {
      if ($scope.fieldValuesData[i].needCorrection) {
        toReject.push($scope.fieldValuesData[i].pk)
      }
    }
    ...
    then(function(response) {
      Flash.create('success', 'Sent Comment Successfully');
      if (response.data.status == 'rejected') {
        $scope.sendbkBtn = false
      }
      $('#codingWindow').scrollTop(0);
