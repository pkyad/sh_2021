...
app.controller("imageViewerCont", function($scope, Flash, $http) {
  $scope.imageViewerUrl = '/static/ngTemplates/configurationManager.html'
})
...
app.controller("controller.imageViewer", function($scope, Flash, $http, $uibModal,$rootScope) {
  ...
  $scope.loadcurrentimg = function(imgUrl,typ){
    ...
    if (typ!=undefined) {
      var imgWidth = Math.round(0.75*screen.width-10)
      var imgHeight = Math.round(screen.height-100)
  ...
  function geturl() {
    ...
    cropped.onload = function() {
      ...
      // console.log(dataURL);
      $scope.dragged = false;
      $scope.sendImageUrl(dataURL, rectangle)
      disabled = false;
  ...
  $scope.fetchConfig = function() {
    ...
    if ($scope.configuration != null) {
      ...
      for (var i = 0; i < $scope.configs.data.length; i++) {
        console.log("page number : " , $scope.configs.data[i].pageNumber);
        console.log("currentPage number : " , $scope.flags.imageIndex);
        if ($scope.configs.data[i].pageNumber == $scope.flags.imageIndex ) {
          canvas.add($scope.configs.data[i].rectMark);
          canvas.add($scope.configs.data[i].rectLabel);
          canvas.add($scope.configs.data[i].labelMark);
        }
    ...
    var configName = window.location.href.split('name=')[1].split('&')[0]

    $http({method : 'GET' , url : '/api/projects/getOCRConfig/?name=' + configName}).
    then(function(response) {
      ...
      for (var i = 0; i < grabs.length; i++) {
        ...
        grabs[i].x = grabs[i].x*canvas.width;
        grabs[i].width = grabs[i].width*canvas.width;
        grabs[i].height = grabs[i].height*canvas.height;
        grabs[i].y = grabs[i].y*canvas.height;

        grabs[i].rect = {top: grabs[i].y , left : grabs[i].x , width : grabs[i].width , height : grabs[i].height}

        grabs[i].rectMark = new fabric.Rect({
          fill: 'transparent',
          stroke: '#337ab7',
          strokeWidth: 2,
          selectable:false,
          ...
          top: grabs[i].y,
          width: grabs[i].width,
          height: grabs[i].height,
        });

        grabs[i].rectLabel = new fabric.Rect({
          fill: '#337ab7',
          stroke: '#337ab7',
          strokeWidth: 2,
          selectable:false,
          visible: true,
          left: grabs[i].x + grabs[i].width,
          top: grabs[i].y,
          width: 35,
          height: grabs[i].height,
        })

        grabs[i].labelMark = new fabric.Text('' + ($scope.configs.data.length+1), {
            left: grabs[i].x + grabs[i].width + 5, //Take the block's position
            top: grabs[i].y,
            fill: 'white',
            fontSize : 25,
            selectable:false,
        ...
        if (grabs[i].pageNumber == 0) {
          canvas.add(grabs[i].rectMark);
          canvas.add(grabs[i].rectLabel);
          canvas.add(grabs[i].labelMark);
        }

        console.log("added " , i);
        $scope.configs.data.push(grabs[i]);
  ...
  $scope.showSelectedText = function(text,url,ocrModelPk , rect) {
    $scope.modelWindow = true
    $uibModal.open({
      templateUrl: '/static/ngTemplates/configManagerCropedText.html',
      size: 'md',
      resolve: {
        data: function() {
          return text;
      ...
      controller: function($scope, data,imgUrl,ocrModelPk, $uibModalInstance,$rootScope,$timeout , showSendBtn , rect) {

        // console.log("in modal window" , rect);

        $scope.form = {text : data , default : '' , label : '', rect : rect}
        $scope.imgUrl = imgUrl
        ...
        $scope.save = function() {
          $uibModalInstance.dismiss({rect: $scope.form.rect ,label : $scope.form.label , default :  $scope.form.default});
        }
    ...
    // canvas.clear()
      ...
      a.pageNumber = $scope.flags.imageIndex;

      a.rect = new fabric.Rect({
        fill: 'transparent',
        stroke: '#337ab7',
        strokeWidth: 2,
        selectable:false,
      ...
      a.rectLabel = new fabric.Rect({
        fill: '#337ab7',
        stroke: '#337ab7',
        strokeWidth: 2,
        selectable:false,
        visible: true,
        left: a.rect.left + a.rect.width,
        top: a.rect.top,
        width: 35,
        height: a.rect.height,
      })

      a.labelMark = new fabric.Text('' + ($scope.configs.data.length+1), {
          left: a.rect.left + a.rect.width + 5, //Take the block's position
          top: a.rect.top,
          fill: 'white',
          fontSize : 25,
          selectable:false,
      })
      canvas.add(a.rectMark);
      canvas.add(a.rectLabel);
      canvas.add(a.labelMark);

      $scope.configs.data.push(a);
  ...
  $scope.edit = function(indx) {
    $uibModal.open({
      templateUrl: '/static/ngTemplates/configManagerEdit.html',
      size: 'md',
      backdrop: true,
      resolve: {
        data: function() {
          return $scope.configs.data[indx]
        }
      },
  ...
  $scope.save = function() {
    ...
    for (var i = 0; i < $scope.configs.data.length; i++) {
      var conf = $scope.configs.data[i];
      var toAdd = {label : conf.label , default : conf.default , x : conf.rect.left/canvas.width , y : conf.rect.top/canvas.height , width : conf.rect.width/canvas.width , height : conf.rect.height/canvas.height , pageNumber : conf.pageNumber }
      toSend.push(toAdd)
    }

    $http({method : 'PATCH' , url : '/api/projects/ocrConfig/'+ $scope.configuration.pk+'/' , data : {grabs : JSON.stringify(toSend) }}).
    then(function(response) {
  ...
  $scope.removeConfig = function(indx) {
    canvas.remove($scope.configs.data[indx].labelMark  );
    canvas.remove($scope.configs.data[indx].rectMark  );
    canvas.remove($scope.configs.data[indx].rectLabel  );
    canvas.renderAll();
    $scope.configs.data.splice(indx , 1);
  }
  ...
  $scope.sendImageUrl = function(url, rect) {
    console.log(ctrlTyp,'controllllll typpppppppppppppp' , rect);
    $http({
      method: 'POST',
      url: '/api/tools/getTextFromImage/',
      data: {
    ...
    then((function(rect) {
      return function(response) {
        ...
        $scope.dragged = false;
        $scope.showSelectedText(response.data.imageText,url,response.data.ocrModelPk , rect)
      }
    })(rect))
  }
  ...
  if (window.location.href.indexOf('file' != -1)) {
    var filePath = window.location.href.split('file=')[1]
    if (filePath != undefined && filePath.length > 0) {
      $scope.sendPDF( filePath ,'imageurl' );
    }else{
      $scope.fileSelectMode = true;
    }
  }

  $scope.fileSelected = new File([""], "");

  $scope.uploadFile = function() {
    var fd = new FormData();

    fd.append('file' , $scope.fileSelected)

    $http({
      method: 'POST',
      url: '/api/projects/saveMediaFile/',
      data: fd,
      transformRequest: angular.identity,
      headers: {
        'Content-Type': undefined
      ...
      window.location.href += "&file=/media/"+response.data.filename
    })
  }
